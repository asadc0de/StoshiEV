...
export default function AdminDashboard() {
  ...
  const silverMembers = usersList.filter(u => u.membershipType === 'Silver' && u.membershipStatus === 'Active').length;
  const goldMembers = usersList.filter(u => u.membershipType === 'Gold' && u.membershipStatus === 'Active').length;
  const platinumMembers = usersList.filter(u => u.membershipType === 'Platinum' && u.membershipStatus === 'Active').length;
  ...
  const handleGeneratePreview = () => {
    ...
    const active = usersList.filter(u => u.membershipStatus === 'Active');
  ...
  const handleLockDistribution = async () => {
    ...
    try {
      ...
      const activeUsers = usersList.filter(u => u.membershipStatus === 'Active');
  ...
  const handleBulkActivate = async () => {
    ...
    try {
      const batch = writeBatch(db);
      const pending = usersList.filter(u => u.membershipStatus === 'Pending' && u.paymentStatus === 'Paid');
            ...
            {activeTab === 'analytics' && (
              <div className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <StatCard icon={<Users className="w-5 h-5" />} label="Total Members" value={loadingData ? '...' : totalUsers} color="text-cyan-400" bgColor="bg-cyan-500/10" borderColor="border-cyan-500/10" />
                  <StatCard icon={<Check className="w-5 h-5" />} label="Active Members" value={loadingData ? '...' : activeMembers} color="text-[#74E61F]" bgColor="bg-[#74E61F]/10" borderColor="border-[#74E61F]/10" />
                  <StatCard icon={<X className="w-5 h-5" />} label="Inactive Members" value={loadingData ? '...' : inactiveMembers} color="text-amber-400" bgColor="bg-amber-500/10" borderColor="border-amber-500/10" />
                  <StatCard icon={<Wallet className="w-5 h-5" />} label="Total Revenue" value={loadingData ? '...' : formatRupee(totalRevenue)} color="text-emerald-400" bgColor="bg-emerald-500/10" borderColor="border-emerald-500/10" />
                </div>
