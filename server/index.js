  ...
  try {
    ...
    if (expectedSignature === razorpay_signature) {
      ...
      try {
        ...
        // 3. Increment projects/sonbhadra-ev1
        const projectRef = db.collection('projects').doc('sonbhadra-ev1');

        const projectSnap = await projectRef.get();
        if (!projectSnap.exists) {
          console.warn(`⚠️ [MISSING PROJECT DOC] projects/sonbhadra-ev1 not found. Creating via set+merge with base values.`);
        }
